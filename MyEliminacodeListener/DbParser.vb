...
Public Class DbParser
    ...
    Public ReadOnly Property DeleteChar() As String
        Get
            Return LDeleteChar
        End Get
    End Property
        ...
        Try
            Select Case TDB
                ...
                Case "1"    'access
                    ...
                    ConnString = "Provider=Microsoft.ACE.OLEDB.16.0;Data Source=" + PathDb + ";Jet OLEDB:Database Password=" + PwdDb + ";" 'User Id=admin;"
    ...
    Public Function EseguiSQL(ByVal StrQ As String, Optional ByRef Righe As Integer = 0) As Boolean
        ...
        Try
            ...
            If LoggaSql Then
                Dim paths As String
                paths = Application.UserAppDataPath & "\logMyQuery.txt"
                Dim fw As New System.IO.StreamWriter(paths, True)
                fw.WriteLine(Now.ToString)
                fw.WriteLine(StrQ)
                fw.Close()
            End If
            Return True
        Catch ex As Exception
            If ex.Message = "ExecuteNonQuery richiede una oggetto Connection aperto e disponibile. Lo stato attuale della connessione è chiuso." Then
                ...
                'End If
            End If
            MessaggioErrore = Now.ToString & ": Errore nell'esecuzione del comando." & ControlChars.CrLf & "Errore restituito: " & ex.Message
            Dim paths As String
            paths = Application.UserAppDataPath & "\logMyErrors.txt"
            Dim fw As New System.IO.StreamWriter(paths, True)
    ...
    Public Function Estrai(ByVal StrQ As String, ByRef ExtDS As DataSet, ByVal NomeTab As String, ByVal Reimposta As Boolean) As Boolean
        ...
        Try
            Select Case TDB
                ...
                Case "1"
                    StrQ = StrQ.Replace("note", "[note]").Replace("Note", "[Note]")
        ...
        Catch ex As Exception
            ...
            If ex.Message = "ExecuteNonQuery richiede una oggetto Connection aperto e disponibile. Lo stato attuale della connessione è chiuso." Or ex.Message = "La connessione è stata disattivata" Then
                'If MsgBox("La connessione è stata interrotta." & vbCrLf & "Provare a riconnettersi?", MsgBoxStyle.Question + MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                MessaggioErrore = Now.ToString & ": Database irraggiungibile." & ControlChars.CrLf & "Errore restituito: " & ex.Message
                fw.WriteLine(MessaggioErrore)
                fw.WriteLine(StrQ)
                Call Connetti()
                ...
                'End If
            End If
            MessaggioErrore = Now.ToString & ": Impossibile estrarre i dati." & ControlChars.CrLf & "Errore restituito: " & ex.Message
            fw.WriteLine(MessaggioErrore)
            fw.WriteLine(StrQ)
            fw.Close()
            Return False
