...
function maxchiamateturno($conn, $idturno) {
	$sql = 'select maxchiamate from turni where ID_turno = ' . $idturno;
	$result = mysqli_query($conn, $sql);
	$row = mysqli_fetch_assoc($result);
	return $row['maxchiamate'];
}
...
// Modalità force: bypassa blocchi orari (solo da LAN, usata dal simulatore in amministrazione.php)
$forceMode = isset($_POST['force']) && $_POST['force'] === '1';
if ($forceMode) {
    $reqIP = $_SERVER['REMOTE_ADDR'] ?? '';
    $lanOk = in_array($reqIP, ['127.0.0.1', '::1'], true)
          || (bool)preg_match('/^(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.)/i', $reqIP);
    if (!$lanOk) $forceMode = false; // ignora force se non è LAN
}

// Verifica se il turno è attivo (saltata in force mode)
$statoTurno = verificaTurnoAttivo($conn, $idturno);

if (!$forceMode && !$statoTurno['attivo']) {
    echo json_encode([
        'success' => false, 
...
$maxnumero = maxchiamateturno($conn, $idturno);
if ($nextnumero > $maxnumero && $maxnumero > 0) {
    echo json_encode(['success' => false, 'error' => 'Il turno è sospeso']);
    mysqli_close($conn);
    exit;	
}
